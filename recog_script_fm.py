...
def face_recognition_call(slidePin, redPin, greenPin):
    ...
    # list for checking all person should find in csf (consicutive frame)
    csf = [[], []]
    ...
    while GPIO.input(slidePin) == 1:
        ...
        if frame is None:
            # print("Unable to load the camera, camera is not connected.")
            # sleep for 10 sec
            time.sleep(10)
            continue
    ...
    # if more than 3 face is found than reduce it to 3 faces
        ...
        # append the names list in csf
        # and remove the first element
        csf.pop(0)
        # append list of names for unknown person
        csf.append(names)
        # print(csf)
        ...
        # for ((top, right, bottom, left), name) in zip(boxes, names):
            ...
            if name != 'Unknown':
                ...
                if (name in csf[-1]) and (name in csf[-2]):
                ...
                # reset csf if same name is found in
                # 3 consicutive frame
                    csf = [[], []]
                    # pass
